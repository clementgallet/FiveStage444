...
public final class Stage2Solver extends StageSolver{
	...
	public void run (){
		while(pullState()) {
			...
			if( StageController.currentStage[ss.id] != 12 ) continue;
			...
			for (goal = cubeDist; goal < cubeDist + 5; ++goal) {
				...
				if( StageController.currentStage[ss.id] != 23 ) break;
			}
		}
	...
	public int getDistanceEdgCen (boolean front){
		...
		cube1.m_centerF = cube.m_centerF;
		cube1.m_centerB = cube.m_centerB;
	...
	public boolean treeSearch (CubeStage2 cube1, int depth, int moves_done, int move_state, int distCenF, int distCenB){
		...
		if (depth == 0) {
			...
			if( StageController.currentStage[ss.id] == 23 ) { 
				pushState();
				return false;
			}
		...
		for (mov_idx = 0; mov_idx < N_STAGE2_SLICE_MOVES; ++mov_idx) {
			if ((stage2_slice_moves_to_try[move_state] & (1 << mov_idx)) != 0) {
				...
				if (treeSearch (cube2, depth - 1, moves_done + 1, next_ms, newDistCenF, newDistCenB)) return true;
			}
		}
		return false;
	}
